...
module elastic_buffer_tb;
  ...
  elastic_buffer #(.DATA_WIDTH(DW), .DEPTH(DEPTH)) dut (.*);
  ...
  initial begin
    ...
    // Single pass
    fork send(8'hA0); recv; join

    // Fill buffer then drain
    ...
    // Burst with out_ready=1
    out_ready=1;
    for (int i=0; i<4; i++) begin
      in_valid=1; in_data=DW'(8'hC0+i);
    ...
    repeat(8) tick;
    out_ready=0;
    if (expected_queue.size()!=0) begin $error("%0d items left", expected_queue.size()); fail_cnt++; end
    else pass_cnt++;

    $display("elastic_buffer_tb: PASS=%0d FAIL=%0d", pass_cnt, fail_cnt);
